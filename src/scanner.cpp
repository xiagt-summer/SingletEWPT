...
void Scanner::ReadScannerParams(std::string fname)
{
	...
	// Boolean flags
	bOnlyXYZ = !!GetFromMap(nums, "onlyXYZ");
	nums.erase("onlyXYZ");
	...
	std::vector<std::string> paramNames{ "mh2", "a1", "a2", "b3", "sinTheta", "T" };
...
void Scanner::FindTransitionPoints() {
	...
	for (int i=0; i < static_cast<int>(resultsForT.size()); i++) {
		...
		if (bFoundTransition) {
			...
			res[2] = GetFromMap(currentInput, "a1_G");
			res[3] = GetFromMap(currentInput, "a2_G");
			res[4] = GetFromMap(currentInput, "sinTheta_G");
			res[5] = GetFromMap(currentInput, "b3_G");
			res[6] = Tc;
			res[7] = vphysByT;
			res[8] = vJump;
			res[9] = xJump;
